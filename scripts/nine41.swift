...
print("Fixing status bars (for iOS 13+)...")
...
var runtimes = json["devices"] as! Dictionary<String, Array<Dictionary<String, Any>>>
for (osVersion, devices) in runtimes {
    runtimes[osVersion] = devices.map { (device: Dictionary<String, Any>) -> Dictionary<String, Any> in
        var mutableDevice = device
        mutableDevice["osVersion"] = osVersion.replacingOccurrences(of: "com.apple.CoreSimulator.SimRuntime.", with: "")
